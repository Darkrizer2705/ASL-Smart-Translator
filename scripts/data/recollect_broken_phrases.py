"""
Recollect data for BROKEN phrases with natural hand movement
...
def collect_with_variation():
    ...
    collected_samples = {phrase: [] for phrase in BROKEN_PHRASES}
    
    print("\n" + "="*70)
    print("RECOLLECTION: Fixing Low-Variance Phrases")
    print("="*70)
    ...
    phrase_idx = 0
    current_phrase = BROKEN_PHRASES[phrase_idx]
    recording = False
    frame_buffer = []
    ...
    while True:
        ...
        cv2.putText(frame, f"Phrase: {current_phrase}", (10, 30),
        ...
        cv2.imshow("Phrase Recollection", frame)
        ...
        if key == ord('q'):
            ...
            recording = False
            phrase_idx = (phrase_idx + 1) % len(BROKEN_PHRASES)
            current_phrase = BROKEN_PHRASES[phrase_idx]
    ...
    if new_rows:
        ...
        for phrase in sorted(combined_df['label'].unique()):
            ...
            status = "✓" if count > 500 else "⚠" if count > 100 else "✗"
