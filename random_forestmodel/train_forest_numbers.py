...
NUMBER_LANDMARKS_CSV = ROOT_DIR / "datasets" / "number_landmarks.csv"
...
def resolve_training_mode(mode: str, image_data_dir: Path, landmarks_csv: Path) -> str:
    ...
    raise FileNotFoundError(
        "No number training data found. Expected either image folders at "
...
def train_number_model(data_dir: Path, model_path: Path) -> None:
    if not data_dir.exists():
        raise FileNotFoundError(
            f"Number image dataset not found: {data_dir}. "
...
def train_number_landmark_model(data_csv: Path, model_path: Path) -> None:
    if not data_csv.exists():
        raise FileNotFoundError(
            f"Number landmarks CSV not found: {data_csv}. "
    ...
    print("Loading number landmarks...")
    df = pd.read_csv(data_csv)
    if df.empty:
        raise ValueError(
            f"Number landmarks CSV has no samples: {data_csv}. "
            "Collect data first with: python src/data/collect_number_landmarks.py"
        )
    if "label" not in df.columns:
        raise ValueError(f"Number landmarks CSV is missing a label column: {data_csv}")
    ...
    save_model_metrics(y_test, y_pred, list(encoder.classes_), "number_landmarks", RESULTS_DIR)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Train number image CNN and/or landmark classifier."
    )
    parser.add_argument(
        "--mode",
        ...
        default="auto",
        help=(
            "Which number model to train. Defaults to auto, which uses the "
            "landmark CSV if present and otherwise falls back to the image CNN."
        ),
    )
    parser.add_argument("--image-data-dir", type=Path, default=Path(NUMBERS_TRAIN_DIR))
    parser.add_argument("--cnn-model-path", type=Path, default=Path(NUMBER_MODEL))
    parser.add_argument("--landmarks-csv", type=Path, default=NUMBER_LANDMARKS_CSV)
    parser.add_argument("--landmark-model-path", type=Path, default=NUMBER_LANDMARK_MODEL)
    ...
    if mode in ("cnn", "both"):
        train_number_model(args.image_data_dir, args.cnn_model_path)

    if mode in ("landmarks", "both"):
        train_number_landmark_model(args.landmarks_csv, args.landmark_model_path)
