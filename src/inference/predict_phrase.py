...
# ── Load Model ─────────────────────────────────────
PHRASE_MODEL_PATH = ROOT_DIR / "models" / "phrase_classifier.pkl"
...
def main():
    ...
    if not PHRASE_MODEL_PATH.exists():
        print("Phrase model not found.")
        print(f"Expected it at: {PHRASE_MODEL_PATH}")
        print("Run: python src/models/train_phrases.py first")
        sys.exit(1)

    with open(PHRASE_MODEL_PATH, "rb") as f:
        data = pickle.load(f)
        model   = data["model"]
